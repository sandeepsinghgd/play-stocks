...
const groupTradingModalSchema = Yup.object().shape({
    ...
    reward: Yup.array(
      Yup.object({
        min_rank: Yup.number()
          .positive()
          .moreThan(0, "Minimum Rank can't be negative.")
        ...
        const lastRange = rewards[lastIndex];
        const lastRank = lastRange.max_rank || lastRange.min_rank;
        const newIndexStartwith = lastRank + 1;
...
const CreateGroupTrading: FC<IEditPublicFormProps> = ({
  ...
  const initialValues = {
    ...
    system_commission: group?.system_commission
      ? group.system_commission
      : "",
    is_guaranteed_return: group?.is_guaranteed_return
      ? group.is_guaranteed_return
      : 0,
    minimum_player: group?.minimum_player ? group.minimum_player : "2",
  ...
  const submit = async (values: any, setFieldError: any) => {
    ...
    values = {
      ...
      minimum_player:
        isGuaranteed && isGuaranteed === 1 ? 2 : values.minimum_player,
    };
  ...
  return (
    <>
      <div className="mb-3">
        <Link to="/trading/group-trading" onClick={()=>{
          setCreateContest(false);
        }}>
      ...
      >
        {({ values, errors, setFieldValue, touched, setFieldTouched }) => (
          <Card title="Group Trading Create Contest" extra="">
            {setMaxRankLimit(values.winning_limit * values.maximum_player /100)}
                  ...
                  <Row>
                    ...
                    <Col xs={12} md={6} lg={6}>
                      <Field
                        ...
                        component={InputField}
                      />
                      {/* {touched.system_commission && errors.system_commission && <p>{errors.system_commission}</p>} */}
                    </Col>
                    {/* <Row> */}
                    <Col xs={12} md={6} lg={6}>
                      <Field
                        ...
                        onChange={(value: any) => {
                          ...
                          const minPlayerVal = value === true ? 2 : minPlayer;
                          if (value === true) {
                                      ...
                                      <Col xs={12} md={6} lg={3}>
                                       ...
                                       {values.reward[index].min_rank &&
                                          maxRankLimit <
                                            values.reward[index].min_rank && (
                                            <p className="text-danger Error-Text ">
                                              Minimum rank should be less then {maxRankLimit}
                                            </p>
                                          )}
                                        {values.reward[index].min_rank &&
                                          maxPlayerVal <
